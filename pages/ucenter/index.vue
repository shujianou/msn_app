	...
	<view>
		<view class="header" v-bind:class="{'status':isH5Plus}">
			<view class="userinfo">
				...
				<view class="info">
					<view class="username" @click="toLogin()">{{userinfo.userName}}</view>
					<view v-if="userinfo.coin!=null" class="integral">牛逼值:{{userinfo.coin}}</view>
				</view>
			</view>
			<view class="setting" @click="toSetting()">
				<image src="../../static/img/ucenter/setting.png"></image>
			</view>
		</view>
		<!-- <view class="orders">
			<view class="box">
				<view class="label" v-for="(row,index) in orderTypeLise" :key="row.name" hover-class="hover" @tap="toOrderType(index)">
					<view class="icon">
						...
						<image :src="'../../static/img/ucenter/'+row.icon"></image>
					</view>
					{{row.name}}
				</view>
			</view>
		...
		<view class="list" v-for="(list,list_i) in severList" :key="list_i">
			<view class="li" v-for="(li,li_i) in list" @tap="toPage(list_i,li_i)" v-bind:class="{'noborder':li_i==list.length-1}"
			 hover-class="hover" :key="li.name">
				<view class="icon">
					<image :src="'../../static/img/ucenter/sever/'+li.icon"></image>
				</view>
				<view class="text">{{li.name}}</view>
				<image class="to" src="../../static/img/ucenter/to.png"></image>
			</view>
		</view>
	</view>
</template>
		...
		methods: {
			init() {
				...
				if (this.userinfo==null) {
					this.userinfo = {
						userName: '登录/注册',
						avatarUrl: '../../static/img/ucenter/face.png'
					}
				}
			},
			//用户点击订单类型
			...
			toLogin() {
				let userinfo = uni.getStorageSync('user').user;
				if (userinfo==null) {
					uni.reLaunch({
